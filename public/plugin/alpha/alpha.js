...
! (function($) {
	...
	$.fn.alpha = function(options) {
		...
		$('.testimonials-wrapper').css('padding-top', maxHeight + 150);
		...
		style += ' #testimonials .testimonial-bubble, #testimonials .testimonial-bubble:after{background: ' + settings.bubbleColor + '}';
		style += ' #testimonials .testimonial-bubble p{color: ' + settings.bubbleText + '}';
		style += ' #testimonials .testimonial-bubble p:after, .testimonial-bubble p:before{color: ' + settings.quoteColor + '}';
		style += '</style>';
		$('#testimonials').closest('html').find('head').append(style);
		...
		if (autoplay === 'on') {
			...
			customer.mouseleave(function() {
				if (autoplay === 'on') play = setInterval(scroll, parseInt(delay));
			});
		}
		...
		customer.mouseenter(function() {
			if (!$(this).closest('#testimonials').hasClass('responsive')) {
				...
				$(this).find('.testimonial-bubble').removeClass('fadeOutUp').addClass('fadeInDown').closest('.customer').find('.cus-profile').find('.cus-name').fadeIn(200,
				function() {
		...
		function calcWidth() {
			...
			$('.cus-image').each(function() {
				tesWidth += $(this).outerWidth(true);
			});
			...
			if ($(window).outerWidth() > 1120 && layout != 'alt') {
				...
				$('#testimonials').find('.testimonial-bubble').each(function() {
					$(this).css({
				...
				// $('#testimonials').removeClass('fixed');//修改部分
