...
//先执行一次
    ...
    $(window).scroll(function () {
        ...
        var pos = (scrollTop+80)/navtoTop;
        var alpha = scrollTop/navtoTop>1?1:scrollTop/navtoTop;
        if(pos>=1){
            pos=1;
            nav.css({"position":"fixed","top":"40px","width":"100%","z-index":2});
        }
        else {
            // console.log("yes")
            nav.css({"position":"absolute","top":0});
        }
        // $("nav .navbar-brand").css({"position":"absolute","left":pos*40+"px"});
        $("nav").css({"background":"RGBA(80,171,226,"+alpha+")"});
        $("nav .setimg").css({"opacity":alpha});
        $("nav .doutu, .video").css({"transform" :"scale("+alpha+")"});
...
//    phone
    $(window).scroll(function () {
        ...
        if (winTop >= 30) {
            $('nav .user').addClass('goleft');
        } else {
            $('nav .user').removeClass('goleft');
        }
    });
